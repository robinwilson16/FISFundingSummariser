CREATE OR ALTER VIEW VW_FIS_LearningAims
AS
	SELECT
		SSA1Code = AIM.SSA1ID,
		...
		LearnAimRef = AIM.LEARNING_AIM_REF,
		LearnAimTitle = AIM.LEARNING_AIM_TITLE,
		...
		NVQLevelOrder = 
			CASE
				...
				WHEN COALESCE ( AIM.NVQLevelID_v2, 'X' ) = 'X' THEN 0.5
				WHEN COALESCE ( AIM.NVQLevelID_v2, 'X' ) = 'H' THEN 4
				WHEN COALESCE ( AIM.NVQLevelID_v2, 'X' ) = 'M' THEN 3
